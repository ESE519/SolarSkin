...
#define MY_MAC_ADDR	0x0001

char *local_rx_buf;
uint8_t length1,trans_flag=0; 
...
void tx_task ()
{
  ...
  while (1) {
    ...
    // bmac_addr_decode_set_my_mac(uint16_t MAC_ADDR); 
     bmac_addr_decode_dest_mac(0x0002);  // 0xFFFF is broadcast
	 val=bmac_tx_pkt(tx_buf, strlen(tx_buf));
    // bmac_addr_decode_enable();
    // bmac_addr_decode_disable();
	 bmac_addr_decode_dest_mac(0x0003);  // 0xFFFF is broadcast
	 val=bmac_tx_pkt(tx_buf, strlen(tx_buf));
/*
