...
char *local_rx_buf;
uint8_t length1,trans_flag=0,pkt_pointer=0;
//char pkt_store_data[max_storage][128];
...
int main(void)

  {
	...
	/*
void push_queue(char *px){
for(int i=0;i<strlen(px);i++){
...
void inter_tx_task ()
{
  ...
  while (1) {
    ...
    bmac_addr_decode_set_my_mac(MY_MAC_ADDR);
	  sprintf(tx_buf,"inter_node1 count:%d",cnt);
...
void tx_task ()
{
  ...
  while (1) {
	...
	bmac_addr_decode_disable();
    bmac_addr_decode_set_my_mac(MY_MAC_ADDR);
	sprintf(tx_buf,"node1 count:%d",cnt);
